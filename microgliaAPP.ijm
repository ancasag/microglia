...
for (j=0; j<files.length; j++) {
	...
	if(endsWith(files[j],".tif"))
	{
		...
		for (m = 0; m < n; m++) {
			...
			contiene = false;
			b = 0;
			while (b < xPoints.length && !contiene) {
				if(Roi.contains(xPoints[b], yPoints[b])){
					contiene=true;
				}
				b++;
			}
...
for (i=0; i<files.length; i++) {
	...
	setResult("Area Nucleo", i, areasN[i]); 
	setResult("Media Nucleo", i, mediasN[i]); 
	setResult("Stds Nucleo", i, stdN[i]); 
	//setResult("Area Ramificaciones", i, areasR[i]); 
	setResult("Media Ramificaciones", i, mediasR[i]); 
	setResult("Stds Ramificaciones", i, stdR[i]); 
}
